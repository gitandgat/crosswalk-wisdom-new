...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { email, firstName, challenge } = req.body;
  ...
  try {
    // 1. Upsert person with challenge stored as custom field
    await enchargeRequest('/people', {
      email,
      firstName,
      workshopChallenge: challenge,
    });

    // 2. Apply tag to trigger waitlist confirmation email
    await enchargeRequest('/tags', {
      tag: 'workshop-waitlist',
      users: [{ email }],
    });
