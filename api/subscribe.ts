...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await enchargeRequest('/people', 'POST', {
      email,
      firstName,
    });

    // 2. Apply tag to trigger the CW Welcome sequence
    // In Encharge: set the "CW Welcome" flow trigger to tag = "crosswalk-community"
    await enchargeRequest('/tags', 'POST', {
      tag: 'crosswalk-community',
      users: [{ email }],
    });
